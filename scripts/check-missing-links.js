...
dataRows.forEach((line, idx) => {
  ...
  dataCells.forEach((cell, colIdx) => {
    ...
    // Check if cell contains plain text models (not wrapped in [name](url))
    // A cell can have: "[ModelA](url) + [ModelB](url)" or "ModelA + ModelB" or mix
    
    // Find all plain text models (not in markdown links)
    const plainTextPattern = /(?<![\[])\b([^\[\]|+]+?)\b(?![\]\(])/g;
    const linkPattern = /\[([^\]]+)\]\(([^)]+)\)/g;
    
    // Extract all linked models
    const linkedModels = [];
    let match;
    while ((match = linkPattern.exec(cell)) !== null) {
      linkedModels.push(match[1].trim());
    }
