...
- **每天 07:30（北京时间）**：`aa-daily.yml` 一站式执行：抓榜单、重建 `aa-ranking.json`、`reorder-vendors.js` 按最新排名重排厂商列、运行 `update-monthly.js` 把新发布的高置信模型自动写入月表，有事时开 Issue 日报（含跳过项、回填候选、缺编者注厂商）
