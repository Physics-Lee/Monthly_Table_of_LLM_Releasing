# AI2 与 Apertus：两个全流程开源透明的机构

> 本笔记整理 AI2（Allen Institute for AI）和 Apertus（Swiss AI Initiative）的基本信息、代表模型及开放性特点。
> 最后更新：2026-09-08
> 更正（2026-09-08）：Apertus 开放指数实为 88.89，与 OLMo 3 并列榜首，此前记录的"满分 100"有误。
...
- **Apertus 70B Instruct**：88.89
- **Apertus 8B Instruct**：88.89

与 AI2 的 OLMo 3/3.1 并列榜首（该指数发布时没有任何模型拿到满分）。两家扣分点完全相同：预训练/后训练数据许可为"商用需署名"档（各得 1/3），权重、方法、数据获取全部满分。
...
2. **完全开源** — 权重+代码+数据全公开
3. **开放性指数极高** — 均为 88.89 分并列榜首，远超 Llama/DeepSeek 的60-70分
...
| **Apertus 70B** | ✅ 合规语料库 | ✅ transformers | ✅ | ✅ 完整配方 | ✅ | 88.89，与 OLMo 3 并列榜首 |
...
| **MiniMax-M1** | ⚠️ 文字描述 | ✅ | ✅ Apache 2.0 | ✅ 训练代码 | ⚠️ 部分 | 有技术博客，无数据脚本 |
| **Step-3.5 Flash (阶跃星辰)** | ⚠️ 文字描述 | ✅ | ✅ Apache 2.0 | ✅ 训练框架 | ⚠️ 部分 | 开源框架，训练数据未公开 |
...
| 等级 | 代表模型 | 特征 |
|------|----------|------|
| **S级（85+）** | OLMo 3, Apertus, K2 Think V2 | 数据/代码/权重/方法全开放，可完整复现，仅数据许可带署名条款 |
